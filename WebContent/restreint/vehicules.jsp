	...
	<div class="container" style="margin:20px" >
		<form action="/TestServletJPA/toto" method="post">
			...
			<fieldset id="common">
				...
				<div class="form-group">
					<label for="immatriculation">Immat :</label><input type="text" id="immatriculation" name="immatriculation" class="form-control" style="width:300px" placeholder="LL-NNN-LL">
				</div>
				<div class="form-group">
					<label for="marque">Marque :</label> <input type="text" class="form-control" id="marque"
						name="marque" style="width:300px" placeholder="Ex: Renault">
				</div>
				<div class="form-group">
					<label for="modele">Modèle :</label> <input type="text" class="form-control" id="modele"
						name="modele" style="width:300px" placeholder="Ex: Clio III">
				</div>
			</fieldset>
			<br />
			<fieldset id="MotoFields">
				...
				<div class="form-group">
					<label for="puissance">Puissance :</label> <input type="text"
						class="form-control" id="puissance" name="puissance" style="width:300px" placeholder="Nombre de CV">
				</div>
			...
			<fieldset id="VoitureFields">
				...
				<div class="form-group">
					<label for="couleur">Couleur :</label> <input type="text"
						id="couleur" class="form-control" name="couleur" style="width:300px">
				</div>
	
				<div class="form-group">
					<label for="annee">Année :</label> <input type="text" class="form-control" id="annee"
						name="annee" style="width:300px" placeholder="YYYY">
				</div>
			</fieldset>
	...
	<table class="table">
		...
		<tbody>
			<c:forEach items="${requestScope.vehicules}" var="vehicule">
				<tr>
					<td scope="row"><c:out value="${vehicule.value.immatriculation}" /></td>
					<td scope="row"><c:out value="${vehicule.value.modele}" /></td>
					<td scope="row"><c:out value="${vehicule.value.nbreRoues}" /></td>
					<td scope="row"><c:out value="${vehicule.value.marque}" /></td>
					<c:choose>
						<c:when test="${vehicule.value.typeVehicule == 'voiture'}">
							<td scope="row"><c:out value="${vehicule.value.couleur}" /></td>
							<td scope="row"></td>
							<td scope="row"><c:out value="${vehicule.value.annee}" /></td>
						</c:when>
						<c:when test="${vehicule.value.typeVehicule =='moto'}">
							<td scope="row"></td>
							<td scope="row"><c:out value="${vehicule.value.puissance}" /></td>
							<td scope="row"></td>
						</c:when>
					</c:choose>
					<td scope="row"><a class="btn btn-danger"
						href="/TestServletJPA/toto/remove?immat=${vehicule.value.immatriculation}">Supprimer</a></td>
				<tr>
			</c:forEach>
		</tbody>
	</table>
